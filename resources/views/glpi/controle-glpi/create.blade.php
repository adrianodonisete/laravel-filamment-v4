@include('layout-twcss.header', ['title' => 'Criar - Controle GLPI'])
...
<div class="flex-1 w-full">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <form method="POST" action="{{ route('glpi.controle-glpi.store') }}" class="space-y-4">
            @csrf
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm mb-1">Ticket</label>
                    <input type="number" name="id_ticket" value="{{ old('id_ticket') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_creation" value="{{ old('date_creation') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                    <label class="block text-sm mb-1">Modificado em</label>
                    <input type="datetime-local" name="date_mod" value="{{ old('date_mod') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                    <label class="block text-sm mb-1">Prioridade Ordem</label>
                    <input type="number" name="priority_order" value="{{ old('priority_order') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                    <label class="block text-sm mb-1">Prioridade Número</label>
                    <input type="number" step="0.000001" name="priority_number" value="{{ old('priority_number') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                    <label class="block text-sm mb-1">Projeto</label>
                    <input type="text" name="proj" value="{{ old('proj') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                    <label class="block text-sm mb-1">Jira</label>
                    <input type="text" name="jira" value="{{ old('jira') }}"
                        class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="note" rows="4" class="w-full rounded border px-3 py-2 bg-white dark:bg-gray-900">{{ old('note') }}</textarea>
                </div>
            </div>
